...
#
# MANUAL EDIT: the tilde must be escaped, and this must be re-applied after
# `mix tinfoil.generate` until joshrotenberg/tinfoil#120 ships. Unescaped, the ~
# in the word of ${var#word} is itself tilde-expanded, so the pattern no longer
# matches, nothing is stripped, and $HOME gets prepended to a still-tilde-
# prefixed path: mkdir then fails on /Users/you~ inside an unwritable /Users.
case "$INSTALL_DIR" in
